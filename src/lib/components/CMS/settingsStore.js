import get from 'npm:just-safe-get';
import set from 'npm:just-safe-set';
...
function _toyStore(init) {
  const {
    ...
    const unsubscribeLS = settingsStore.subscribe(val =>
...
export const pauseFlash = writable(false);
export const settingsStore = _toyStore(JSON.parse(localStorage.getItem('toyStore')) || init);