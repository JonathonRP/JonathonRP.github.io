<script>
  ...
  import { settingsStore, pauseFlash, settingsTitles } from './settingsStore';
  import AddItem from './AddField.svelte';
  ...
  $: {
    if (keyRef && $settingsStore.flashUpdates && !inSettings) {
...
<span
  ...
  {#if $settingsStore.advancedEdit && !inSettings}
    <button
      class="deleteItem"
      title={`Delete ${path}`}
      on:click={() => {
        doPauseFlash();
        settingsStore.updateExternalStore({
          store,
          path,
          key: array.index,
          value: null,
  ...
  {#if bounds.length}
    ...
    {/if}
  {/if}
  {#if $settingsStore.advancedEdit && bounds.length}
