...
export class ComparerBuilder<T>
  implements BaseComparerBuilder<T>, ThenComparerBuilder<T> {
...
export default class OrderedIterable<T> extends GeneratorIterable<T> {
  ...
  public thenBy<TCmp>(selector: (x: T) => TCmp): OrderedIterable<T> {
    ...
    const newBuilder = this.comparerBuilder.thenKey((x) =>
      x.orders[this.depth + 1]
    );
  ...
  public thenByDescending<TCmp>(selector: (x: T) => TCmp): OrderedIterable<T> {
    ...
    const newBuilder = this.comparerBuilder.thenKeyDescending((x) =>
      x.orders[this.depth + 1]
    );
    return new OrderedIterable(wrapped, newBuilder, this.depth + 1);
  }
...
function orderBy<T, TCmp>(
  ...
  const builder = ComparerBuilder.create<OrderedItem<T>>().sortKey((x) =>
    x.orders[0]
  );
