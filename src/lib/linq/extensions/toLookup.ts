...
import { Enumerable, type EqualityComparer, type IndexedSelector } from '../Enumerable';
...
const isValueSelector = <T, TValue>(obj: any): obj is IndexedSelector<T, TValue> => typeof obj === 'function';
...
function toLookup<T, TKey, TValue = TKey>(
	...
	for (const item of this) {
		...
		const arr: ArrayIterable<T | TValue> = map.get(key) || new ArrayIterable<T | TValue>([]);
		map.set(key, arr);
