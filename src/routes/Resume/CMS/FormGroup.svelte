<script>
  ...
  $: {
    if ($store) {
      ...
      if ($settingsStore.autoExpand && filter) {
  ...
  onMount(() => {
    ...
    return store.subscribe(() => {
      isSectionOpen = $settingsStore.autoExpand && displayedPrior ? true : isSectionOpen;
      displayedPrior = true;
    });
  });
...
{#if isSectionOpen}
  <div class="state-mang-data" transition:slide>
    {#if addItemsExpanded && $toyStore.advancedEdit}
      <AddField {store} path={''} bind:show={addItemsExpanded} />
    {/if}
    {#if typeof $store === 'object'}
      {#each storeKV as [key, value]}
        <Field {store} path={key} {key} {value} {filter} {showComplex} {inSettings} />
      {/each}
    {:else}
      <Field
        {store}
